...
def run_single_fire_simulation(
    b0, # Changed
    initial_stocks_value, initial_bonds_value, initial_str_value, initial_fun_value, initial_real_estate_value, # Changed
    t_ret_months, t_ret_years, # Changed
    x_real_monthly_initial, # Changed
    c_planned, # Changed
    x_planned_extra, # Changed
    p_real_monthly, pension_inflation_adjustment_factor, y_p_start_idx, # Changed
    s_real_monthly, salary_inflation_adjustment_factor, y_s_start_idx, y_s_end_idx, # Changed
    mu_log_pi, sigma_log_pi,
    rebalancing_year_idx, # Changed
    w_p1_stocks, w_p1_bonds, w_p1_str, w_p1_fun, w_p1_real_estate, # Changed
    w_p2_stocks, w_p2_bonds, w_p2_str, w_p2_fun, w_p2_real_estate, # Changed
    ...
    real_bank_lower_bound, # Changed
    real_bank_upper_bound, # Changed
    c_real_monthly_initial, # Changed
    h0_real_cost, # Changed
    ter_annual_percentage, # Changed
    shock_events, # Changed
):
    """
    ...
    current_bank_balance = b0 # Changed
    current_stocks_value = initial_stocks_value # Changed
    current_bonds_value = initial_bonds_value # Changed
    current_str_value = initial_str_value # Changed
    current_fun_value = initial_fun_value # Changed
    current_real_estate_value = initial_real_estate_value # Changed

    # Initialize current weights for all assets (Phase 1 weights initially)
    current_weights_stocks = w_p1_stocks # Changed
    current_weights_bonds = w_p1_bonds # Changed
    current_weights_str = w_p1_str # Changed
    current_weights_fun = w_p1_fun # Changed
    current_weights_real_estate = w_p1_real_estate # Changed

    # Initialize normalized weights for investment (from Phase 1 liquid weights)
    liquid_weights_sum = (
        w_p1_stocks + w_p1_bonds + # Changed
        w_p1_str + w_p1_fun # Changed
    )
    if liquid_weights_sum > 0:
        normalized_weights_stocks = w_p1_stocks / liquid_weights_sum # Changed
        normalized_weights_bonds = w_p1_bonds / liquid_weights_sum # Changed
        normalized_weights_str = w_p1_str / liquid_weights_sum # Changed
        normalized_weights_fun = w_p1_fun / liquid_weights_sum # Changed
    else:
    ...
    nominal_wealth_history = np.zeros(t_ret_months) # Changed
    bank_balance_history = np.zeros(t_ret_months)   # Changed
    ...
    annual_inflations_sequence = np.random.lognormal(
        mu_log_pi, sigma_log_pi, t_ret_years # Changed
    ) - 1
    annual_stocks_returns_sequence = np.random.lognormal(
        mu_log_stocks, sigma_log_stocks, t_ret_years # Changed
    ) - 1
    annual_bonds_returns_sequence = np.random.lognormal(
        mu_log_bonds, sigma_log_bonds, t_ret_years # Changed
    ) - 1
    annual_str_returns_sequence = np.random.lognormal(
        mu_log_str, sigma_log_str, t_ret_years # Changed
    ) - 1
    annual_fun_returns_sequence = np.random.lognormal(
        mu_log_fun, sigma_log_fun, t_ret_years # Changed
    ) - 1
    annual_real_estate_returns_sequence = np.random.lognormal(
        mu_log_real_estate, sigma_log_real_estate, t_ret_years # Changed
    ) - 1

    # Apply SHOCK_EVENTS to the pre-generated annual sequences
    for shock in shock_events: # Changed
        shock_year = shock['year']
        shock_asset = shock['asset']
        shock_magnitude = shock['magnitude']

        if 0 <= shock_year < t_ret_years: # Changed
    ...
    cumulative_inflation_factors_annual = np.ones(t_ret_years + 1) # Changed
    for year_idx in range(t_ret_years): # Changed
        cumulative_inflation_factors_annual[year_idx + 1] = (
            cumulative_inflation_factors_annual[year_idx] * (1 + annual_inflations_sequence[year_idx])
        )

    # --- OPTIMIZATION 2: Pre-calculate all monthly returns for all assets ---
    monthly_returns_lookup = {
        'Stocks': np.zeros(t_ret_months), # Changed
        'Bonds': np.zeros(t_ret_months),  # Changed
        'STR': np.zeros(t_ret_months),    # Changed
        'Fun': np.zeros(t_ret_months),    # Changed
        'Real Estate': np.zeros(t_ret_months) # Changed
    }
    ...
    for year_idx in range(t_ret_years): # Changed
        ...
        end_month = min((year_idx + 1) * 12, t_ret_months) # Changed
    ...
    for real_amount, year_idx in c_planned: # Changed
        nominal_contribution_amount = real_amount * cumulative_inflation_factors_annual[year_idx]
        nominal_planned_contributions_amounts.append((nominal_contribution_amount, year_idx))

    nominal_planned_extra_expenses_amounts = []
    local_planned_extra_expenses = list(x_planned_extra) # Changed
    for real_amount, year_idx in local_planned_extra_expenses:
        nominal_extra_expense_amount = real_amount * cumulative_inflation_factors_annual[year_idx]
    ...
    nominal_pension_annual_sequence = np.zeros(t_ret_years) # Changed
    nominal_salary_annual_sequence = np.zeros(t_ret_years) # Changed

    for year_idx in range(t_ret_years): # Changed
        if year_idx >= y_p_start_idx: # Changed
            # Calculate the pension adjustment factor
            if year_idx > y_p_start_idx: # Changed
                pension_adjusted_inflations = (
                    annual_inflations_sequence[y_p_start_idx:year_idx] * pension_inflation_adjustment_factor # Changed
                )
            ...
            nominal_pension_annual_sequence[year_idx] = (
                p_real_monthly # Changed
                * cumulative_inflation_factors_annual[y_p_start_idx] # Changed
                * pension_factor
            )

        if y_s_start_idx <= year_idx < y_s_end_idx: # Changed
            # Calculate the salary adjustment factor
            if year_idx > y_s_start_idx: # Changed
                salary_adjusted_inflations = (
                    annual_inflations_sequence[y_s_start_idx:year_idx] * salary_inflation_adjustment_factor # Changed
                )
            ...
            nominal_salary_annual_sequence[year_idx] = (
                s_real_monthly # Changed
                * cumulative_inflation_factors_annual[y_s_start_idx] # Changed
                * salary_factor
            )

    ter_monthly_factor = ter_annual_percentage / 12.0 # Changed

    # Simulation loop
    for current_month_idx in range(t_ret_months): # Changed
        ...
        if month_in_year_idx == 0:
            if current_year_idx < rebalancing_year_idx: # Changed
                current_weights_stocks = w_p1_stocks # Changed
                current_weights_bonds = w_p1_bonds # Changed
                current_weights_str = w_p1_str # Changed
                current_weights_fun = w_p1_fun # Changed
                ...
                current_weights_stocks = w_p2_stocks # Changed
                current_weights_bonds = w_p2_bonds # Changed
                current_weights_str = w_p2_str # Changed
                current_weights_fun = w_p2_fun # Changed
                current_weights_real_estate = w_p2_real_estate # Changed

            liquid_weights_sum = (
                current_weights_stocks + current_weights_bonds +
                current_weights_str + current_weights_fun
            )
            normalized_weights_stocks = (
                current_weights_stocks / liquid_weights_sum if liquid_weights_sum > 0 else 0
            )
            normalized_weights_bonds = (
                current_weights_bonds / liquid_weights_sum if liquid_weights_sum > 0 else 0
            )
            normalized_weights_str = (
                current_weights_str / liquid_weights_sum if liquid_weights_sum > 0 else 0
            )
            normalized_weights_fun = (
                current_weights_fun / liquid_weights_sum if liquid_weights_sum > 0 else 0
            )


        # 1. Add pension if applicable
        nominal_pension_monthly = 0
        if current_year_idx >= y_p_start_idx: # Changed
        ...
        if y_s_start_idx <= current_year_idx < y_s_end_idx: # Changed
        ...
        if current_real_bank_balance < real_bank_lower_bound: # Changed
            real_shortfall_to_cover = real_bank_lower_bound - current_real_bank_balance # Changed
        ...
        if current_real_bank_balance > real_bank_upper_bound: # Changed
            real_excess_to_invest = current_real_bank_balance - real_bank_upper_bound # Changed
        ...
        for i, (nominal_contribution_amount, contribution_year_idx) in enumerate(c_planned): # Changed
            if (current_year_idx == contribution_year_idx
                ...
                current_real_estate_value += nominal_contribution_amount * current_weights_real_estate
                planned_contribution_applied_for_year = True

        # Apply C_REAL_MONTHLY_INITIAL every month
        if c_real_monthly_initial > 0: # Changed
            nominal_monthly_contribution = (
                c_real_monthly_initial * cumulative_inflation_factors_annual[current_year_idx] # Changed
            )
        ...
        nominal_monthly_expenses = (
            x_real_monthly_initial * cumulative_inflation_factors_annual[current_year_idx] # Changed
        )
        ...
        for i, (nominal_extra_expense_amount, expense_year_idx) in enumerate(local_planned_extra_expenses):
        ...
        current_real_estate_value *= (1 + monthly_returns_lookup['Real Estate'][current_month_idx])

        # Apply TER
        current_stocks_value *= (1 - ter_monthly_factor) # Changed
        current_bonds_value *= (1 - ter_monthly_factor)  # Changed
        current_str_value *= (1 - ter_monthly_factor)    # Changed
        current_fun_value *= (1 - ter_monthly_factor)    # Changed
        ...
        if (current_year_idx == rebalancing_year_idx # Changed
            and month_in_year_idx == 0
            and rebalancing_year_idx > 0): # Changed
            total_investment_value_pre_rebalance = (
                current_stocks_value + current_bonds_value + current_str_value +
                current_fun_value + current_real_estate_value
            )

            cumulative_inflation_rebalance_year = (
                cumulative_inflation_factors_annual[rebalancing_year_idx] # Changed
            )

            pre_rebalancing_allocations_nominal = {
                'Stocks': current_stocks_value, 'Bonds': current_bonds_value, 'STR': current_str_value,
                'Fun': current_fun_value, 'Real Estate': current_real_estate_value
            }
            pre_rebalancing_allocations_real = {
                ...
                'Real Estate': current_real_estate_value / cumulative_inflation_rebalance_year
            }

            # --- START HOUSE PURCHASE LOGIC ---
            if h0_real_cost > 0: # Changed
                nominal_house_cost = (
                    h0_real_cost * cumulative_inflation_factors_annual[rebalancing_year_idx] # Changed
                )
            ...
            if h0_real_cost > 0: # Changed
                ...
                sum_liquid_p2_weights = (
                    w_p2_stocks + w_p2_bonds + # Changed
                    w_p2_str + w_p2_fun # Changed
                )
                ...
                if sum_liquid_p2_weights == 0:
                    ...
                    normalized_weights_phase2_stocks = w_p2_stocks / sum_liquid_p2_weights # Changed
                    normalized_weights_phase2_bonds = w_p2_bonds / sum_liquid_p2_weights # Changed
                    normalized_weights_phase2_str = w_p2_str / sum_liquid_p2_weights # Changed
                    normalized_weights_phase2_fun = w_p2_fun / sum_liquid_p2_weights # Changed

                    current_stocks_value = liquid_portfolio_value_for_rebalance * normalized_weights_phase2_stocks
                    current_bonds_value = liquid_portfolio_value_for_rebalance * normalized_weights_phase2_bonds
                    current_str_value = liquid_portfolio_value_for_rebalance * normalized_weights_phase2_str
                    current_fun_value = liquid_portfolio_value_for_rebalance * normalized_weights_phase2_fun
                ...
                rebalancing_allocations_nominal = {
                    'Stocks': total_investment_value_pre_rebalance * w_p2_stocks, # Changed
                    'Bonds': total_investment_value_pre_rebalance * w_p2_bonds, # Changed
                    'STR': total_investment_value_pre_rebalance * w_p2_str, # Changed
                    'Fun': total_investment_value_pre_rebalance * w_p2_fun, # Changed
                    'Real Estate': total_investment_value_pre_rebalance * w_p2_real_estate # Changed
                }
        ...
        nominal_wealth_history[current_month_idx] = (
            current_bank_balance + current_stocks_value + current_bonds_value +
            current_str_value + current_fun_value + current_real_estate_value
        )
    ...
    cumulative_inflation_end_of_sim = cumulative_inflation_factors_annual[t_ret_years] # Changed
    final_allocations_real = {
    ...
    return (success, months_lasted, final_investment_value, 
            final_bank_balance, annual_inflations_sequence,
